...
export class Message extends Model{
    ...
    //Analisar o tipo da mensagem
    getViewElement(me = true){//por padrão, a mensagem é minha
        ...
        switch (this.type){
            ...
            div.querySelector('.message-photo').on('load', e=>{
                ...
                div.querySelector('._340lu').hide()//oculta a foto de carregamento
                div.querySelector('._3v3PK').css({
                    height: 'auto' //formata o tamanho da mensagem pelo tamanho da imagem
                })
            })
            break;
            ...
            default: //Em caso de mensagens de texto
                div.innerHTML = `
        ...
        if(me){//se for minha mensagem
            className = 'message-out' //Mensagem recebida

            //Adiciona o status
            div.querySelector('.message-time').parentNode.appendChild(this.getStatusViewElement())
        }
        div.firstElementChild.classList.add(className) //classList adiciona mais classe sem perder as outras
        return div
    }
    ...
    static sendContacts(chatId, from, contact){
    ...
    static sendImage(chatId, from, file){
        return new Promise((s,f)=>{
            ...
            Message.upload(file, from).then(snapshot =>{
                Message.send(//envio da mensagem
                    chatId, 
                    from, 
                    'image', 
    ...
    static send(chatId, from, type,content){
        return new Promise((s,f)=>{
            Message.getRef(chatId).add({
                ...
                let docRef = result.parent.doc(result.id)//retorna o id da mensagem
                
                docRef.set({
                    status: 'sent'
                }, {
                    merge: true //muda os status e não excluir as outras informações
